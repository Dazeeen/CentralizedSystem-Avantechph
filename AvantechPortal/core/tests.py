...
from .forms import AssetAccountabilityForm, AssetItemForm, FundRequestForm, RoleForm, StaffUserCreationForm, StaffUserUpdateForm, prepare_image_upload
...
class StaffUserUpdateFormPasswordTests(TestCase):
    def _base_form_data(self, user, password):
        return {
    ...
    def test_update_form_does_not_require_admin_current_password_field(self):
        target = get_user_model().objects.create_user(username='target-user', password='OldPassword12345')
        admin = get_user_model().objects.create_superuser(username='admin-user', password='AdminPassword12345')

        form = StaffUserUpdateForm(instance=target, current_user=admin)

        self.assertNotIn('admin_current_password', form.fields)

    def test_superuser_can_set_short_or_simple_password(self):
        target = get_user_model().objects.create_user(username='target-user', password='OldPassword12345')
        admin = get_user_model().objects.create_superuser(username='admin-user', password='AdminPassword12345')
        form = StaffUserUpdateForm(
            data=self._base_form_data(target, 'abc'),
            instance=target,
            current_user=admin,
        )
        ...
        self.assertTrue(updated_user.check_password('abc'))

    def test_non_superuser_must_follow_password_validation(self):
        target = get_user_model().objects.create_user(username='target-user', password='OldPassword12345')
...
class StaffUserCreationFormPasswordTests(TestCase):
    def _base_form_data(self, username, password):
        return {
            'username': username,
            'first_name': '',
            'last_name': '',
    ...
    def test_superuser_can_create_user_with_short_or_simple_password(self):
        admin = get_user_model().objects.create_superuser(username='admin-user', password='AdminPassword12345')
        form = StaffUserCreationForm(
            data=self._base_form_data('new-target-user', 'abc'),
            current_user=admin,
        )
        ...
        self.assertTrue(created_user.check_password('abc'))

    def test_non_superuser_creation_must_follow_password_validation(self):
        staff_user = get_user_model().objects.create_user(
            username='staff-user',
            password='StaffPassword12345',
            is_staff=True,
...
class PrivateChatMessageWindowTests(TestCase):
    def setUp(self):
        self.department = Group.objects.create(name='Chat QA')
        self.sender = get_user_model().objects.create_user(username='chat-sender', password='password')
        self.recipient = get_user_model().objects.create_user(username='chat-recipient', password='password')
        self.conversation = PrivateChatConversation.objects.create(
            department=self.department,
            participant_one=self.sender,
            participant_two=self.recipient,
        )

    def _create_messages(self, count):
        messages = []
        for index in range(count):
            message = PrivateChatMessage(conversation=self.conversation, sender=self.sender)
            message.set_message(f'Message {index + 1}')
            message.save()
            messages.append(message)
        return messages

    def test_message_window_hides_paging_at_ten_messages(self):
        self._create_messages(10)

        payload = views._private_chat_message_window(self.conversation, self.sender, self.recipient)
    ...
    def test_message_window_pages_only_after_ten_messages(self):
        messages = self._create_messages(11)

        latest_payload = views._private_chat_message_window(self.conversation, self.sender, self.recipient)
        older_payload = views._private_chat_message_window(
            self.conversation,
            self.sender,
            self.recipient,
            before_id=latest_payload['oldest_id'],
        )

        self.assertEqual(latest_payload['count'], 10)
        self.assertEqual(latest_payload['total_count'], 11)
        self.assertTrue(latest_payload['has_previous'])
        self.assertFalse(latest_payload['has_next'])
        self.assertEqual(older_payload['messages'][0]['id'], messages[0].pk)
        self.assertFalse(older_payload['has_previous'])
        self.assertTrue(older_payload['has_next'])


class PrivateChatMessagesViewTests(TestCase):
    def test_messages_view_returns_empty_history_without_existing_conversation(self):
        department = Group.objects.create(name='Chat View QA')
        viewer = get_user_model().objects.create_user(username='chat-viewer', password='password')
        member = get_user_model().objects.create_user(username='chat-member', password='password')
        viewer.groups.add(department)
        member.groups.add(department)

        self.client.force_login(viewer)
        response = self.client.get(
            reverse('chats_messages'),
            {'department': department.pk, 'user': member.pk, 'mark_read': '1'},
        )
...
class AssetItemParentTypeTests(TestCase):
    ...
    def test_assets_search_includes_other_item_type(self):
        admin = get_user_model().objects.create_superuser(username='asset-super', password='password')
        AssetItemType.objects.update_or_create(
            code='other',
            defaults={'name': 'Other', 'prefix': 'AST', 'is_active': True},
        )
        other_item = AssetItem.objects.create(
            department=self.department,
            item_name='Unlabeled Adapter',
            item_type='other',
            code_prefix='AST',
            stock_quantity=1,
            low_stock_threshold=1,
        )
        monitor_item = AssetItem.objects.create(
            department=self.department,
        ...
        self.client.force_login(admin)
        response = self.client.get(reverse('assets_list'), {'q': 'others'})

        self.assertEqual(response.status_code, 200)
        self.assertIn(other_item, list(response.context['items_page'].object_list))
        self.assertNotIn(monitor_item, list(response.context['items_page'].object_list))

    def test_consumables_search_includes_other_item_type(self):
        admin = get_user_model().objects.create_superuser(username='consumable-super', password='password')
        ConsumableItemType.objects.create(name='Other', code='other', prefix='CON', is_active=True)
        other_item = ConsumableItem.objects.create(
            department=self.department,
            item_name='Misc Refill',
            item_type='other',
            code_prefix='CON',
            stock_quantity=1,
            low_stock_threshold=1,
        )
        paper_type = ConsumableItemType.objects.create(name='Paper', code='paper', prefix='PPR', is_active=True)
        paper_item = ConsumableItem.objects.create(
            department=self.department,
            item_name='Bond Paper',
            item_type=paper_type.code,
            code_prefix='PPR',
            stock_quantity=1,
            low_stock_threshold=1,
        )
        ...
        self.client.force_login(admin)
        response = self.client.get(reverse('consumables_list'), {'q': 'others'})

        self.assertEqual(response.status_code, 200)
        self.assertIn(other_item, list(response.context['items_page'].object_list))
        self.assertNotIn(paper_item, list(response.context['items_page'].object_list))

    def test_borrow_item_search_labels_include_other_item_type(self):
        AssetItemType.objects.update_or_create(
            code='other',
            defaults={'name': 'Other', 'prefix': 'AST', 'is_active': True},
        )
        ...
        form = AssetAccountabilityForm(request_user=self.user)
        label = form.fields['item'].label_from_instance(other_item)

        self.assertIn('Other', label)


class AccountingRequestAccessTests(TestCase):
    def setUp(self):
        User = get_user_model()
        self.requester = User.objects.create_user(username='requester-a', password='password')
        self.other_requester = User.objects.create_user(username='requester-b', password='password')
        self.accounting_user = User.objects.create_user(username='accounting-viewer', password='password')
        self.approver = User.objects.create_user(username='accounting-approver', password='password')
        add_perm = Permission.objects.get(codename='add_accountingrequest')
        view_fund_perm = Permission.objects.get(codename='view_fundrequest')
        approve_perm = Permission.objects.get(codename='approve_accountingrequest')
        self.requester.user_permissions.add(add_perm)
        self.accounting_user.user_permissions.add(view_fund_perm)
        self.approver.user_permissions.add(add_perm, approve_perm)
        self.own_request = AccountingRequest.objects.create(
            request_type='purchase_requisition',
            requester_name='Requester A',
            title='Requester A PR',
            description='Materials request',
            created_by=self.requester,
        )
        self.other_request = AccountingRequest.objects.create(
            request_type='job_request',
            requester_name='Requester B',
            title='Requester B JO',
            description='JO request',
            created_by=self.other_requester,
        )

    def test_requester_only_sees_own_accounting_requests(self):
        self.client.force_login(self.requester)

        response = self.client.get(reverse('accounting_requests'))

        self.assertEqual(response.status_code, 200)
        rows = list(response.context['pending_requests_page'].object_list)
        self.assertIn(self.own_request, rows)
        self.assertNotIn(self.other_request, rows)

    def test_accounting_viewer_sees_all_requests_but_cannot_approve(self):
        ...
        rows = list(response.context['pending_requests_page'].object_list)
        self.assertIn(self.own_request, rows)
        self.assertIn(self.other_request, rows)
        self.assertFalse(response.context['can_approve_accounting_requests'])

    def test_approver_can_approve_accounting_request(self):
        self.client.force_login(self.approver)

        response = self.client.post(
            reverse('accounting_requests'),
            {'action_type': 'approve_request', 'request_id': str(self.own_request.pk)},
        )

        self.assertRedirects(response, reverse('accounting_requests'))
        self.own_request.refresh_from_db()
        self.assertEqual(self.own_request.request_status, 'approved')
        self.assertEqual(self.own_request.processed_by, self.approver)
...
class FileManagerTrashTests(TestCase):
    ...
    def test_regular_user_sees_only_owned_and_shared_file_manager_items(self):
        self.owner.user_permissions.add(Permission.objects.get(codename='view_managedfilenode'))
        other_user = get_user_model().objects.create_user(username='other-user', password='password')
        outside_folder = ManagedFileNode.objects.create(
            owner=self.owner,
            name='Outside Root',
            node_type='folder',
            access_scope='private',
        )
        private_other_file = ManagedFileNode.objects.create(
            parent=self.parent,
            owner=other_user,
            name='Other Private.docx',
            node_type='file',
            access_scope='private',
            file=SimpleUploadedFile('other-private.docx', b'private'),
        )
        owned_file = ManagedFileNode.objects.create(
            parent=self.parent,
            owner=self.owner,
            name='My File.docx',
            node_type='file',
            access_scope='private',
            file=SimpleUploadedFile('my-file.docx', b'owned'),
        )
        owned_outside_scope_file = ManagedFileNode.objects.create(
            parent=outside_folder,
            owner=self.owner,
            name='Outside Scope.docx',
            node_type='file',
            access_scope='private',
            file=SimpleUploadedFile('outside-scope.docx', b'outside'),
        )
        shared_folder = ManagedFileNode.objects.create(
            parent=self.parent,
            owner=other_user,
            name='Shared Work',
            node_type='shared_folder',
            access_scope='shared',
        )
        shared_file = ManagedFileNode.objects.create(
            parent=shared_folder,
            owner=other_user,
            name='Shared File.docx',
            node_type='file',
            access_scope='shared',
            file=SimpleUploadedFile('shared-file.docx', b'shared'),
        )
        ManagedFilePermission.objects.create(node=shared_folder, user=self.owner, access_level='read')

        parent_response = self.client.get(reverse('file_manager_list'), {'node': outside_folder.pk})
        user_folder_response = self.client.get(reverse('file_manager_list'), {'node': self.parent.pk})
        shared_response = self.client.get(reverse('file_manager_list'), {'node': shared_folder.pk})
        search_response = self.client.get(reverse('file_manager_search'), {'q': 'File'})
        private_download_response = self.client.get(
            reverse('file_manager_download', args=[private_other_file.pk]),
            HTTP_X_REQUESTED_WITH='XMLHttpRequest',
        )
        outside_scope_download_response = self.client.get(
            reverse('file_manager_download', args=[owned_outside_scope_file.pk]),
            HTTP_X_REQUESTED_WITH='XMLHttpRequest',
        )
        shared_download_response = self.client.get(reverse('file_manager_download', args=[shared_file.pk]))

        self.assertEqual(parent_response.context['selected_node'].pk, self.parent.pk)
        self.assertContains(user_folder_response, owned_file.name)
        self.assertContains(user_folder_response, shared_folder.name)
        self.assertNotContains(parent_response, private_other_file.name)
        self.assertNotContains(parent_response, owned_outside_scope_file.name)
        self.assertContains(shared_response, shared_file.name)
        self.assertNotContains(search_response, private_other_file.name)
        self.assertNotContains(search_response, owned_outside_scope_file.name)
        self.assertContains(search_response, shared_file.name)
        self.assertEqual(private_download_response.status_code, 403)
        self.assertEqual(outside_scope_download_response.status_code, 403)
        self.assertEqual(shared_download_response.status_code, 200)

    def test_regular_user_indexes_shared_folders_outside_own_folder(self):
        self.owner.user_permissions.add(Permission.objects.get(codename='view_managedfilenode'))
        other_user = get_user_model().objects.create_user(username='shared-owner', password='password')
        other_root = ManagedFileNode.objects.create(
            owner=other_user,
            name='Shared Owner Root',
            node_type='folder',
            access_scope='private',
        )
        shared_folder = ManagedFileNode.objects.create(
        ...
        shared_file = ManagedFileNode.objects.create(
            parent=shared_folder,
            owner=other_user,
            name='Client Shared Scope File.docx',
            node_type='file',
            access_scope='shared',
            file=SimpleUploadedFile('client-shared-scope-file.docx', b'shared'),
        )
        ManagedFilePermission.objects.create(node=shared_folder, user=self.owner, access_level='read')

        default_response = self.client.get(reverse('file_manager_list'), {'node': self.parent.pk})
        shared_response = self.client.get(reverse('file_manager_list'), {'node': shared_folder.pk})
        search_response = self.client.get(
            reverse('file_manager_search'),
            {'q': 'Client Shared', 'scope': self.parent.pk},
        )

        self.assertContains(default_response, shared_folder.name)
        self.assertEqual(shared_response.context['selected_node'].pk, shared_folder.pk)
        self.assertContains(shared_response, shared_file.name)
        self.assertContains(search_response, shared_folder.name)
        self.assertContains(search_response, shared_file.name)

    def test_regular_user_cannot_open_default_hierarchy_folders_directly(self):
        admin = get_user_model().objects.create_superuser(username='admin', password='password')
        self.owner.user_permissions.add(Permission.objects.get(codename='view_managedfilenode'))
        views._ensure_file_manager_default_hierarchy()
        branch_node = ManagedFileNode.objects.filter(parent__isnull=True, owner=admin).exclude(name__iexact='Trash').first()
        user_folder = ManagedFileNode.objects.get(owner=self.owner, name=self.owner.username)

        response = self.client.get(reverse('file_manager_list'), {'node': branch_node.pk})

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['selected_node'].pk, user_folder.pk)
        self.assertNotIn(branch_node.pk, [node.pk for node in response.context['root_nodes']])
        self.assertNotContains(response, f'?node={branch_node.pk}')
    ...
    def test_file_manager_setup_is_admin_only(self):
        self.owner.user_permissions.add(
            Permission.objects.get(codename='view_managedfilenode'),
            Permission.objects.get(codename='add_managedfilenode'),
            Permission.objects.get(codename='change_managedfilenode'),
        )

        page_response = self.client.get(reverse('file_manager_list'), {'node': self.parent.pk})
        setup_response = self.client.post(
            reverse('file_manager_setup'),
            {'setup_action': 'quota', 'target_user_id': self.owner.pk, 'capacity_mb': '100'},
            HTTP_X_REQUESTED_WITH='XMLHttpRequest',
        )

        self.assertNotContains(page_response, 'data-bs-target="#fileManagerSetupModal"')
        self.assertNotContains(page_response, 'id="fileManagerSetupModal"')
...
class FileManagerUploadTests(TestCase):
    def setUp(self):
        ...
        ManagedUserStorageQuota.objects.create(user=self.user, capacity_mb=0, updated_by=self.owner)
    ...
    def test_user_without_storage_limit_cannot_upload(self):
        ManagedUserStorageQuota.objects.filter(user=self.user).delete()
        self.client.force_login(self.user)
        ...
        self.assertIn('storage capacity is set', response.json().get('message', ''))
        self.assertFalse(ManagedFileNode.objects.filter(parent=self.folder, name='notes.txt').exists())

    def test_user_without_storage_limit_cannot_create_folder(self):
        ManagedUserStorageQuota.objects.filter(user=self.user).delete()
        self.client.force_login(self.user)

        response = self.client.post(
            reverse('file_manager_create_folder'),
            {'parent_id': str(self.folder.id), 'folder_name': 'Blocked Folder'},
            HTTP_X_REQUESTED_WITH='XMLHttpRequest',
        )
        ...
        self.assertFalse(response.json()['ok'])
        self.assertIn('storage capacity is set', response.json().get('message', ''))
        self.assertFalse(ManagedFileNode.objects.filter(parent=self.folder, name='Blocked Folder').exists())
